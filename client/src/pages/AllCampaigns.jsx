...
//               handleCampaignClick(params.row.id); // Pass the entire row data
//             }}
//           >
//             {params.value}
//           </span>
...
//     return allCampaigns?.map((data, index) => ({
//       sno: index + 1,
//       id: data?.id,
//       name: data?.name || "-",
//       priority: "Medium",
...
const AllCampaigns = () => {
  ...
  // Fetch campaigns on component mount and when workspace_id changes
  useEffect(() => {
    if (workspace_id) {
      dispatch(getAllCampaigns(workspace_id));
    }
  ...
  // Properly memoized row data transformation
  const filteredRows = useMemo(() => {
  ...
  }, [allCampaigns]); // Add allCampaigns as dependency
