...
const Sidebar = ({ isOpen, setIsOpen, collapsed, setCollapsed }) => {
  ...
  return (
    <>
      ...
      <div
        ref={sidebarRef}
        className={`fixed top-0 left-0 h-full bg-gray-800 text-gray-100 shadow-xl z-30 transition-all duration-300 ease-in-out ${
          collapsed ? "w-16" : "w-64"
        } max-w-[80vw] md:translate-x-0`}
      >
        {/* Logo & Collapse Toggle */}
        <div className="flex items-center justify-between px-3 py-4 border-b border-gray-700 relative">
          {!collapsed && (
            ...
            {menuItems.map((item, index) => (
              <li key={index}>
                {item.dropdown ? (
                  ...
                  >
                    <span className="text-gray-300">{item.icon}</span>
                    {!collapsed && <span className="text-gray-100">{item.title}</span>}
                  </Link>
                )}
              </li>
            ))}
